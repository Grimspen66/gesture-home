    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                curr_sign = keypoint_classifier_labels[hand_sign_id]
                ...
                if prev_sign:
                    ...
                    if gesture_pair in combo_map and current_time - last_time <= 1.5:
                        ...
                        # fire smartthings command
                        if action in ("on", "off"):
                            if loop and loop.is_running():
                                print("sending command...")
                                asyncio.run_coroutine_threadsafe(do_smartthings(action), loop)
                            else:
...
async def do_smartthings(action):
    try:
        token = '8ea553fa-3e1e-450e-9a9a-38389c3896dc'  # Your access token
        async with aiohttp.ClientSession() as session: #sets to run with smartthings api
            api = pysmartthings.SmartThings(session, token)
            devices = await api.devices() #gest list of devices
            print(f"found some smartthings stuff")
            for d in devices:
            #checks status, sends on or off signal if detected
                print(f"device -> {d.label}")
                if action == "off":
                    await d.status.refresh()
                    print(f"firing: {action.upper()} for device '{d.label}'")
                    await d.switch_off()
                elif action == "on":
                    await d.status.refresh()
                    print(f"firing: {action.upper()} for device '{d.label}'")
        ...
        print(f"crash @ smartthings {e}")

# background event loop (hope this holds)
import threading
loop = None
...
def start_background_loop():
    ...
    asyncio.set_event_loop(loop)  #make it current
    print("[DEBUG] Asyncio loop starting...")
    try:
        loop.run_forever() #keep loop alive

    except Exception as e:
        print(f"loop gave up: {e}")
